...
So sánh out/30M-p2 (baseline) vs out/30M-raft. Output trieulh/report/data/grpo_judge_eval.json.
"""
...
import os
PROG = "data/raft/grpo_eval_progress.jsonl"
done = {}
...
pf = open(PROG, "a")
for name, path in [("grpoA", "out/30M-grpo"), ("grpoB", "out/30M-grpo")]:
    tok = m = None
    ov, adh = [], []
    for i, r in enumerate(tests):
        ...
        story = gen(tok, m, cond, seed=(1234 if name == "grpoA" else 4321) + i)
...
json.dump(res, open("trieulh/report/data/grpo_judge_eval.json", "w"), ensure_ascii=False)
