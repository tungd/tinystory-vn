...
So sánh out/30M-p2 (baseline) vs out/30M-raft. Output trieulh/report/data/raft_judge_eval.json.
"""
...
import os
PROG = "data/raft/eval_progress.jsonl"
done = {}
...
pf = open(PROG, "a")
for name, path in [("p2", "out/30M-p2"), ("raft", "out/30M-raft")]:
    tok = m = None
    ov, adh = [], []
...
json.dump(res, open("trieulh/report/data/raft_judge_eval.json", "w"), ensure_ascii=False)
