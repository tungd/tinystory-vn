...
So sánh out/30M-p2 (baseline) vs out/30M-raft. Output trieulh/report/data/big_judge_eval.json.
"""
...
import os
PROG = "data/raft/big_eval_progress.jsonl"
done = {}
...
pf = open(PROG, "a")
for name, path in [("p2", "out/30M-p2"), ("grpo", "out/30M-grpo")]:
    tok = m = None
    ov, adh = [], []
...
json.dump(res, open("trieulh/report/data/big_judge_eval.json", "w"), ensure_ascii=False)
