"""Continue v3-full on the cleaner, wider v4 target set."""
...
def resolve_max_steps(train_examples: int, batch: int, requested: int) -> int:
    if requested < 0:
        raise ValueError("max steps cannot be negative")
    return requested or math.ceil(train_examples / batch)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--data", default="runs/v4/data")
    parser.add_argument("--base-model", default="runs/v3/artifacts/hf")
    parser.add_argument("--out", default="runs/v4/artifacts/hf")
    ...
    if args.train_samples:
        datasets["train"] = datasets["train"].select(
            range(min(args.train_samples, len(datasets["train"])))
        )
    ...
    max_steps = resolve_max_steps(len(encoded["train"]), args.batch, args.max_steps)
    ...
    # Start from v3-full weights with a fresh optimizer/scheduler.
    trainer.train()
    trainer.save_model(str(output))
    tokenizer.save_pretrained(str(output))
    (output / "v4_training_config.json").write_text(
